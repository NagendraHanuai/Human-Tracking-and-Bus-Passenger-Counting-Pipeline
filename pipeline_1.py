...
VIDEO_PATH = r"videos_data/GX010077.MP4"
OUTPUT_PATH = "videos_data/results_data/G5_GX010077.MP4"
JSON_OUTPUT_PATH = "videos_data/results_data/G5_GX010077_.json"
...
LINE_LAYOUT_MODE = "double"  # "auto", "single", or "double"
...
def get_corrected_person_id(raw_track_id, bbox, current_frame_person_ids):
    ...
    for person_id, last_box in person_last_boxes.items():
        ...
        if iou >= ID_RELINK_IOU or dist_ratio <= ID_RELINK_CENTER_DIST:
            score = iou + (1 - min(dist_ratio, 1))
            if score > best_score:
                best_score = score
...
# -------- JSON FRAME DATA EXTRACTION -------------
        ...
        detected_box_count = len(results.boxes) if results.boxes is not None else 0
        ...
        if results.boxes is not None:
            for box in results.boxes:
                ...
                if track_id is not None:
                    person_id = get_corrected_person_id(track_id, bbox, current_person_ids)
